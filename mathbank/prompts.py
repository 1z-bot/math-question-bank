...
COMMON_OCR_PROMPT = (
    ...
    "2. 变量与坐标包裹：几何点符号、单个字母变量及坐标表达式（如 $(1,2)$）必须严格包裹单美元符号 $...$。下标、上标、函数、方程、不等式、集合、区间和几何符号同样必须完整置于数学环境中，例如 `$x_1$`、`$y^2$`、`$x \\geqslant 0$`、`$f(x_1) \\leqslant f(x_2)$`、`$(0,+\\infty)$`、`$\\triangle PQR$`；绝对不得把这些 LaTeX 片段裸露在普通文本中。\n"
    "2.1 向量字形：原图中的粗体斜体向量字母必须转写为数学环境中的 `\\boldsymbol{...}`，例如 `$\\boldsymbol{a}$`；不得用会变成粗体正体的 `\\mathbf{a}` 代替。\n"
...
def build_pdf_parse_system_prompt(curriculum: dict, generate_answers_bool: bool) -> str:
    ...
    system_instructions = (
        ...
        "3. 公式格式化与排版环境：选择题选项统一格式化为 `\\begin{choices} \\item ... \\end{choices}` 环境；填空题下划线统一使用标准的 `\\fillin` 宏；文本加粗必须使用 `\\textbf{...}`（严禁双星号 `**`）。`content` 与 `answer_markdown` 中所有数学片段都必须使用行内 `$...$` 或独立 `$$...$$` 定界，包含单字母变量、上下标、函数、坐标、方程、不等式、集合、区间、分式和几何符号。正确示例：`$x_1$`、`$y^2$`、`当 $x \\geqslant 0$ 时`、`$f(x_1) \\leqslant f(x_2)$`、`$C: \\frac{x^2}{a^2}+\\frac{y^2}{b^2}=1$`、`$\\triangle PQR$ 的面积`；禁止输出裸露的 `x_1`、`y^2`、`\\frac`、`\\geqslant`、`\\subseteq` 或 `\\triangle`。\n"
        "3.1 向量字形：题面中的粗体斜体向量字母使用 `$\\boldsymbol{a}$`；不得误写成粗体正体 `$\\mathbf{a}$`。只有原文明确表示正体粗体符号时才保留 `\\mathbf`。\n"
